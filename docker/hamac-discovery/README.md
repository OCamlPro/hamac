...
Tag canonique : `registry.ocamlpro.com/hamac-discovery:0.1.0` + `latest`.

## Build

```sh
./build.sh                                  # tag par défaut
./build.sh registry.ocamlpro.com/hamac-discovery:dev   # tag custom
```
...
docker run --rm -p 8877:8877 \
  ...
  registry.ocamlpro.com/hamac-discovery:0.1.0
```
...
docker push registry.ocamlpro.com/hamac-discovery:0.1.0
docker push registry.ocamlpro.com/hamac-discovery:latest
```
