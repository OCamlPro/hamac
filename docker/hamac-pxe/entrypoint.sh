...
# ---- dnsmasq.conf -----------------------------------------------------------
{
  ...
  echo ""
  echo "# pxe-service : sans ça, un dhcp-boot seul ne suffit pas en mode"
  echo "# proxy quand un vrai DHCP (la Turris) répond aussi sur le même"
  echo "# segment — dnsmasq reconnaît le vendor-class PXEClient mais ne"
  echo "# construit/n'envoie jamais de réponse ProxyDHCP (confirmé par"
  echo "# strace : aucun sendto() après le recvmsg de la requête). Les"
  echo "# sous-options PXE (option 43) générées par pxe-service sont ce"
  echo "# qui manquait. Limité à tag:!ipxe (avant chargement d'iPXE) pour"
  echo "# ne pas interférer avec le dhcp-boot=...,boot.ipxe du 2e étage."
  echo "pxe-service=tag:hamac-pxe,tag:!ipxe,X86-64_EFI,\"hamac-pxe boot\",ipxe.efi"
  echo "pxe-service=tag:hamac-pxe,tag:!ipxe,x86PC,\"hamac-pxe boot\",undionly.kpxe"
  echo ""
  echo "# pxe-prompt (prompt vide, timeout 0) : sans ça, dnsmasq laisse le"
  echo "# firmware PXE croire qu'une négociation \"boot server discovery\""
  echo "# (requête unicast port 4011) est nécessaire avant de choisir un"
  echo "# service. Confirmé par strace : le client envoie bien sa requête"
  echo "# sur :4011, dnsmasq la recvmsg() correctement, mais ne construit"
  echo "# jamais de réponse (aucun sendto derrière), donc 4 retries client"
  echo "# puis abandon. timeout=0 signale au client qu'il n'y a qu'un seul"
  echo "# service et qu'il doit booter directement dessus, sans passer par"
  echo "# ce round-trip :4011."
  echo "pxe-prompt=,0"
  echo ""
  ...
  echo "# Log-facility : stderr uniquement (pas de syslog). L'image ne fournit"
  echo "# pas de /dev/log (pas de syslogd dans le container) — sans ce réglage,"
  echo "# dnsmasq tente un connect() vers /dev/log à chaque paquet PXE, qui"
  echo "# échoue (ENOENT, vu en strace) ; suspect dans un cas où une requête"
  echo "# PXEClient:Arch:00007 était bien reconnue (log DHCP présent) mais"
  echo "# jamais suivie d'un sendto() de réponse."
  echo "log-facility=-"
